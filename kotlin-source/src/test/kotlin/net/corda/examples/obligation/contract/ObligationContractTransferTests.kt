...
class ObligationContractTransferTests : ObligationContractUnitTests() {
    ...
    fun `must handle multiple command values`() {
        ledger {
            ...
            transaction {
                input(INSURECAR_CONTRACT_ID) { oneDollarObligation }
                output(INSURECAR_CONTRACT_ID) { oneDollarObligation.withNewLender(CHARLIE) }
    ...
    fun `must have one input and one output`() {
        ledger {
            ...
            transaction {
                input(INSURECAR_CONTRACT_ID) { oneDollarObligation }
                output(INSURECAR_CONTRACT_ID) { oneDollarObligation.withNewLender(CHARLIE) }
                output(INSURECAR_CONTRACT_ID) { DummyState() }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ObligationContract.Commands.Transfer() }
                this `fails with` "An obligation transfer transaction should only create one output state."
            }
            transaction {
    ...
    fun `only the lender may change`() {
        ledger {
            transaction {
                input(INSURECAR_CONTRACT_ID) { Obligation(10.DOLLARS, ALICE, BOB) }
                output(INSURECAR_CONTRACT_ID) { Obligation(1.DOLLARS, ALICE, BOB) }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ObligationContract.Commands.Transfer() }
                this `fails with` "Only the lender property may change."
            }
            transaction {
                input(INSURECAR_CONTRACT_ID) { Obligation(10.DOLLARS, ALICE, BOB) }
                output(INSURECAR_CONTRACT_ID) { Obligation(10.DOLLARS, ALICE, CHARLIE) }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ObligationContract.Commands.Transfer() }
                this `fails with` "Only the lender property may change."
            }
            transaction {
                input(INSURECAR_CONTRACT_ID) { Obligation(10.DOLLARS, ALICE, BOB, 5.DOLLARS) }
                output(INSURECAR_CONTRACT_ID) { Obligation(10.DOLLARS, ALICE, BOB, 10.DOLLARS) }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ObligationContract.Commands.Transfer() }
                this `fails with` "Only the lender property may change."
            }
            transaction {
