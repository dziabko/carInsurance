...
class ObligationContractIssueTests : ObligationContractUnitTests() {
    ...
    fun `issue obligation transaction must have no inputs`() {
        ledger {
            transaction {
                input(INSURECAR_CONTRACT_ID) { DummyState() }
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { oneDollarObligation }
    ...
    fun `Issue transaction must have only one output obligation`() {
        ledger {
            transaction {
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { oneDollarObligation } // Two outputs fails.
                output(INSURECAR_CONTRACT_ID) { oneDollarObligation }
                this `fails with` "Only one obligation state should be created when issuing an obligation."
            }
            transaction {
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { oneDollarObligation } // One output passes.
                this.verifies()
            }
        }
    }
    ...
    fun `cannot issue zero value obligations`() {
        ledger {
            transaction {
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { Obligation(0.POUNDS, ALICE, BOB) } // Zero amount fails.
                this `fails with` "A newly issued obligation must have a positive amount."
            }
            transaction {
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { Obligation(100.SWISS_FRANCS, ALICE, BOB) }
                this.verifies()
            }
            transaction {
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { Obligation(1.POUNDS, ALICE, BOB) }
                this.verifies()
            }
            transaction {
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { Obligation(10.DOLLARS, ALICE, BOB) }
                this.verifies()
            }
        }
    }
    ...
    fun `lender and borrower cannot be the same`() {
        ...
        ledger {
            transaction {
                command(ALICE_PUBKEY, BOB_PUBKEY) { ObligationContract.Commands.Issue() }
                output(INSURECAR_CONTRACT_ID) { borrowerIsLenderObligation }
